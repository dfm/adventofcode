...
class Grid {
  public:
  Grid() : current_index_(0), current_({0, 0}) {
  ...
  bool move(const Coord<T> &step, const T &value) {
    current_ += step;
    ...
    if (current_index_ >= coords_.size() || coords_[current_index_] != current_) {
      ...
      values_.insert(values_.begin() + current_index_, value);
      return true;
    }
    values_[current_index_] = std::min(values_[current_index_], value);
    return false;
  }

  size_t traverse() const {
    ...
    std::queue<Coord<T>> nodes;
    size_t stride = max_x - min_x + 1;
    std::vector<T> full_grid(stride * (max_y - min_y + 1));
    size_t index = size - 1;
    for (T y = max_y; y >= min_y; --y) {
      for (T x = max_x; x >= min_x; --x) {
        full_grid[(y - min_y) * stride + (x - min_x)] = 0;
        auto coord = coords_[index];
        if (x == coord.x && y == coord.y) {
          if (values_[index] == -1) {
            full_grid[(y - min_y) * stride + (x - min_x)] = -1;
          } else if (values_[index] == -2) {
            nodes.push({x - min_x, y - min_y, 0});
            full_grid[(y - min_y) * stride + (x - min_x)] = 1;
          }
          index--;
        }
    ...
    while (!nodes.empty()) {
      Coord current = nodes.front();
      nodes.pop();
      full_grid[current.y * stride + current.x] = current.distance;
      if (current.distance > max_distance) {
        max_distance = current.distance;
      }
      for (int i = 0; i < 4; ++i) {
        auto new_ind = (current.y + cols[i]) * stride + (current.x + rows[i]);
        if (current.y + cols[i] >= 0 && current.x + rows[i] >= 0 && new_ind < full_grid.size() && full_grid[new_ind] == 0) {
          nodes.push({current.x + rows[i], current.y + cols[i], current.distance + 1});
        }
      }
    }
  ...
  friend std::ostream &operator<<(std::ostream &os, const Grid<T> &grid) {
    ...
    for (T y = max_y; y >= min_y; --y) {
      for (T x = max_x; x >= min_x; --x) {
        ...
        if (x == coord.x && y == coord.y) {
          if (grid.values_[index] == -1) {
            os << "#";
          } else if (grid.values_[index] == -2) {
            os << "F";
          } else if (grid.values_[index] == 0) {
            os << "S";
          } else {
            os << " ";
          }
...
class Robot {
  public:
  Robot(char *filename) : program(filename, input){};
  ...
  void explore(int current_dir = 0, size_t distance = 0) {
    for (int dir = 1; dir <= 4; ++dir) {
      if (opposite[current_dir - 1] == dir) continue;
      input.push(dir);
      program.run();
      T code = program.output().front();
      program.output().pop();

      if (code) {
        if (code == 2) {
          std::cout << "distance: " << (distance + 1) << std::endl;
          grid.move(direction[dir - 1], -2);
        } else {
          grid.move(direction[dir - 1], distance + 1);
        }

        // Recursively explore
        explore(dir, distance + 1);

        // Return to starting point
        input.push(opposite[dir - 1]);
        program.run();
        program.output().pop();
        grid.move(direction[opposite[dir - 1] - 1], distance);
      } else if (code == 2) {
  ...
  Coord<T> direction[4] = {{0, 1}, {0, -1}, {-1, 0}, {1, 0}};
  int opposite[4]       = {2, 1, 4, 3};
};
