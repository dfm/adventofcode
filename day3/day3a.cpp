...
size_t manhattan_distance(long int x, long int y) { return std::abs(x) + std::abs(y); }
...
class Path {
  ...
  void move(const std::string &token) {
    ...
    for (size_t i = 0; i < distance; ++i) {
      ...
      current.distance = manhattan_distance(current.x, current.y);
...
int main() {
  ...
  std::cout << find_shortest_distance(matches) / 2 << std::endl;
  return 0;
}